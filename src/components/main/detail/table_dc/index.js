...
import TableDC from './table_dc'
import RowDC from './row_dc'

import helperAction from '../../../../actions/helper_action'
import oddAction from '../../../../actions/odd_action'

import { sortDcLeague } from '../../selector'

let { expireCookies } = config

class TableDCComponent extends Component{
    ...
    render(){ 
        let { DCOdd, tableDouble, tabLinkActive, isSortByLeague, resultSports } = this.props;
        if(!Helper.checkObj(tableDouble)) tableDouble = {}
        let list_item = [];
        list_item = DCOdd.map((item, index) => {
...
export default connect(mapStateToProps, mapDispatchToProps)(TableDCComponent)
